...
const fetchHistoryDataFromAPI = async (page = 1, pageSize = 10) => {
  const response = await fetch(`http://192.168.0.108:8001/history?page=${page}&limit=${pageSize}`);
  if (!response.ok) {                //改本身電腦IP
...
*/
  ...
  const renderHistoryItem = ({ item }) => (
    <View style={styles.historyItem}>
      <Text style={styles.historyDate}>{item.now_time}</Text>
