...
export default function cal() {
  ...
  const getCurrentScoreInfo = (score) => {
    ...
    if (score < 0) return { range: '<0', level: '惡劣', color: '#ff4757' };
  ...
  const fetchHistory = async () => {
    setLoading(true);
    try {
      const data = await fetch('http://192.168.0.108:8001/history?page=1&limit=20')
        .then(res => res.json());
      setHistoryData(data);
    } catch (err) {
  ...
  const safeHistory = historyData?.length ? historyData : [{ temperature: 0, ph: 0, do: 0, score: 0 }];

  return {
    labels: safeHistory.map((_, i) => `${i + 1}`),
    datasets: [
      {data: safeHistory.map(item => safeValue(item.temperature)), color: () => 'red', strokeWidth: 3},
      {data: safeHistory.map(item => safeValue(item.ph)),color: () => 'yellow',strokeWidth: 3},
      {data: safeHistory.map(item => safeValue(item.do)),color: () => 'cyan',strokeWidth: 3},
      {data: safeHistory.map(item => safeValue(item.score)),color: () => 'purple',strokeWidth: 3}
    ],
    legend: ['Temperature', 'PH', 'DO', 'Score'],
  };
}, [historyData]);
  ...
  return (
    ...
    >
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={true}>
        ...
        <View>
          <LineChart style={styles.line_chart}
            data={chartData}
            width={width * 0.90}
            height={220}
            chartConfig={{
